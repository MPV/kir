...
`,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := FindImages(decode(t, tt.manifest))
...
`
	...
	if got := FindImages(decode(t, rollout)); !slices.Equal(got, want) {
...
`,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := FindImages(decode(t, tt.manifest)); len(got) != 0 {
...
`
	...
	for range 50 {
		if got := FindImages(doc); !slices.Equal(got, want) {
