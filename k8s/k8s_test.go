...
`,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := infer(decode(t, tt.manifest))
...
`
	...
	if got := infer(decode(t, rollout)); !slices.Equal(got, want) {
		t.Errorf("infer() = %v, want %v", got, want)
	}
}
...
`,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := infer(decode(t, tt.manifest)); len(got) != 0 {
				t.Errorf("infer() = %v, want no images", got)
			}
		})
	}
}
...
`
	...
	for range 50 {
		if got := infer(doc); !slices.Equal(got, want) {
			t.Fatalf("infer() = %v, want %v — order is not stable", got, want)
		}
	}
}
