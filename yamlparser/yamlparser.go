...
// container images of every workload it contains, recognising PodSpecs with the
// given matcher. Documents are separated using the Kubernetes YAML reader,
// which correctly handles leading and trailing "---" separators, separators
// followed by trailing whitespace, CRLF line endings, and a final document
...
func ProcessReader(matcher *k8s.Matcher, r io.Reader) ([]string, error) {
	var images []string
	var errs []error
	...
	for {
		...
		imgs, err := ProcessData(matcher, doc)
		if err != nil {
...
// ProcessData returns the images in a single manifest document, recognising
// PodSpecs with the given matcher.
//
// The document is decoded into plain Go values rather than into a typed
// Kubernetes object, and the PodSpec is then found by schema. Nothing consults
// a list of kinds: a document yields images if it contains something the schema
// recognises, whether it is a Deployment, a CronJob, a List, or a custom
// resource the Kubernetes scheme has never heard of.
func ProcessData(matcher *k8s.Matcher, data []byte) ([]string, error) {
	...
	return matcher.FindImages(doc), nil
}
